...
`packer` does not provide any commands out of the box, only functions. As such, you may wish to add
some commands to control common operations, such as the following. Note that these commands assume
you have a `plugins.lua` similar to the above:
```vim
...
The above snippets give some examples of `packer` features and use. You can find another (stupid)
example of use in `test_init.vim` and a more realistic example [in my
dotfiles](https://github.com/wbthomason/dotfiles/blob/linux/neovim/.config/nvim/lua/plugins.lua)
(also [this
file](https://github.com/wbthomason/dotfiles/blob/linux/neovim/.config/nvim/plugin/plugins.vim);
[here's](https://github.com/wbthomason/dotfiles/blob/linux/neovim/.config/nvim/plugin/packer_load.vim)
an example of what `packer`'s autogenerated lazy-loader code looks like)

The following is a more in-depth explanation of `packer`'s features and use.

### Initialization
Load `packer` like any other Lua module. You must call `packer.init()` before performing any
operations; it is recommended to call `packer.reset()` if you may be re-running your specification
code (e.g. by sourcing your plugin specification file with `luafile`).
